...
The following is needed for the SC compiler. Without machine/endian.h (GUSI),
the compiler warns, that there are no prototypes for the ntohs() and ntohl() 
functions. Later, when linking, ILink can't resolve the references to the
ntohs and ntohl symbols and fails (although they are in the PerlStub library).
So we better include the machine/endian.h header to make SC and ILink happy.
